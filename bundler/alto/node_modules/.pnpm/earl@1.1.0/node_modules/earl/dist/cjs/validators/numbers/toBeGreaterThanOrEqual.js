"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.toBeGreaterThanOrEqual = void 0;
const expect_js_1 = require("../../expect.js");
const index_js_1 = require("../../format/index.js");
const greaterThanOrEqual_js_1 = require("../../matchers/numbers/greaterThanOrEqual.js");
(0, expect_js_1.registerValidator)('toBeGreaterThanOrEqual', toBeGreaterThanOrEqual);
function toBeGreaterThanOrEqual(control, target) {
    const actualInline = (0, index_js_1.formatCompact)(control.actual);
    const targetInline = (0, index_js_1.formatCompact)(target);
    control.assert({
        success: (0, greaterThanOrEqual_js_1.greaterThanOrEqual)(target)(control.actual),
        reason: `The value ${actualInline} is not greater than or equal to ${targetInline}, but it was expected to be.`,
        negatedReason: `The value ${actualInline} is greater than or equal to ${targetInline}, but it was expected not to be.`,
    });
}
exports.toBeGreaterThanOrEqual = toBeGreaterThanOrEqual;
//# sourceMappingURL=toBeGreaterThanOrEqual.js.map
...
export function formatUnknown(value, sibling, options, valueStack, siblingStack) {
    ...
    switch (type) {
        ...
        case 'symbol':
            return formatSymbol(value, sibling);
    }
    ...
    const items = [];
    const { typeName, isDifferentPrototype, isSameTypeName } = getComparedTypeName(value, sibling, type, options.ignorePrototypes);
    if (typeName) {
        items.push(typeName);
    }
    ...
    if (type === 'Error' &&
        ...
        return toLine(`${typeName !== null && typeName !== void 0 ? typeName : ''}(${formatString(value.message, options)})`);
    }
    ...
    if (type === 'Array') {
        entries.push(...formatArrayEntries(value, sibling, options, valueStack, siblingStack));
    }
    else if (type === 'Set') {
        entries.push(...formatSetEntries(value, sibling, options, valueStack, siblingStack));
    }
    else if (type === 'Map') {
        entries.push(...formatMapEntries(value, sibling, options, valueStack, siblingStack));
    }
    entries.push(...formatObjectEntries(value, sibling, options, valueStack, siblingStack));
    valueStack.pop();
    siblingStack.pop();
